...
def doTargetPrediction(pickled_model_name):
	...
	probs2 = clf.predict_proba(querymatrix2)[:,1]
	preds2 = sum(probs2 > threshold)
	...
	try:
		...
		return ratio, uniprot, preds1, preds1_percentage, preds2, preds2_percentage, oddsratio, pvalue
...
out_file.write('Uniprot\tPref_Name\tGene ID\tTarget_Class\tOrganism\t'+input_name1+'_Hits\t'+input_name1+'_Precent_Hits\t'+input_name2+'_Hits\t'+input_name2+'_Precent_Hits\tOdds_Ratio\tFishers_Test_pvalue\tPrediction_Ratio\n')
...
output_name = input_name1 + '_vs_' + input_name2 + '_out_enriched_diseases_' + str(threshold) + '_' + str(dgn_threshold) + '.txt'
out_file = open(output_name, 'w')
